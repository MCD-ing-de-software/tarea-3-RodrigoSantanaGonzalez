...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        (usar numpy.testing.assert_allclose() para comparar arrays de NumPy - esto es mejor 
        que unittest porque maneja la comparación de arrays numéricos con tolerancia para errores
         de punto flotante)
        - Verificar que el resultado tiene la forma (shape) esperada (usar self.assertEqual 
        para comparar tuplas de .shape - comparación simple, unittest es suficiente)
        """
        arr = [1, 2, 3, 4]
        utils = StatisticsUtils()

        resultado=utils.moving_average(arr, window=2)
        expectado=np.array([1.5, 2.5, 3.5])

        npt.assert_allclose(resultado, expectado, rtol=1e-7, atol=1e-7)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        - Llamar a moving_average con window=0 (valor no positivo) y verificar que se lanza un 
        ValueError (usar self.assertRaises)
        - Llamar a moving_average con window mayor que la longitud del array y verificar
         que se lanza un ValueError (usar self.assertRaises)
        """
        arr=[1,2,3]
        utils = StatisticsUtils()
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        - Llamar a moving_average con esa secuencia y verificar que se 
        lanza un ValueError indicando que solo se aceptan secuencias 1D (usar self.assertRaises)
        """
        arr=[[1, 2, 4],[4,5,6]]
        utils = StatisticsUtils()
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        (usar self.assertAlmostEqual para un solo valor numérico - unittest es suficiente)
        """
        arr=[10,20,30,40]
        utils = StatisticsUtils()
        resultado=utils.zscore(arr)

        self.assertAlmostEqual(resultado.mean(), 0.0, places=7)
        self.assertAlmostEqual(resultado.std(), 1.0, places=7)
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        - Llamar a zscore con esa secuencia y verificar que se lanza un ValueError indicando 
        que la desviación estándar es cero (usar self.assertRaises)
        """
        arr=[5,5,5]
        utils = StatisticsUtils()
        with self.assertRaises(ValueError):
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        - Verificar que el valor mínimo del resultado es 0.0 (usar self.assertAlmostEqual 
        para un solo valor numérico - unittest es suficiente)
        - Verificar que el valor máximo del resultado es 1.0 (usar self.assertAlmostEqual 
        para un solo valor numérico - unittest es suficiente)
        - Verificar que los valores transformados son correctos (ej: [0.0, 0.5, 1.0] para 
        [2, 4, 6]) (usar numpy.testing.assert_allclose() para comparar el array completo - 
        esto es necesario para comparar arrays de NumPy con tolerancia para errores de punto flotante)
        """
        arr=[2,4,6]
        utils = StatisticsUtils()
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        - Llamar a min_max_scale con esa secuencia y verificar que se lanza un ValueError 
        indicando que todos los valores son iguales (usar self.assertRaises)
        """
